...
sf::Sprite& SpriteManager::CreateSprite(std::string _name, std::string _filepath, fCOORD _scale, fCOORD _pos) {
	...
	sprite.setPosition(_pos.X - sprite.getGlobalBounds().width, _pos.Y - sprite.getGlobalBounds().height);
...
void SpriteManager::CreateSprites(string _path, fCOORD _scale, fCOORD _pos)
{
	for (auto const& imgPath : std::filesystem::directory_iterator{ _path })
		CreateSprite(imgPath.path().stem().string(), imgPath.path().string(), _scale, _pos);
}
